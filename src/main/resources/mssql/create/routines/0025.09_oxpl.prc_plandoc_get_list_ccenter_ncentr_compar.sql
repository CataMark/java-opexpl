create or alter procedure oxpl.prc_plandoc_get_list_ccenter_ncentr_compar
    @main_set int,
    @compar_set int,
    @hier char(5),
...
as
    with recs as (
        ...
        inner join oxpl.fnc_hier_get_childs(@hier, @main_set, @cost_centre, @kid) as c
        ...
        where a.data_set = @main_set and a.hier = @hier and
        ...
        group by a.coarea, a.hier, a.data_set, d.cost_driver, e.nume, e.central, a.opex_categ, d.nume, g.blocat, a.ic_part, f.nume,
                cast((case when a.coarea = f.coarea then 1 else 0 end) as bit), b.an
        ...
        inner join oxpl.fnc_hier_get_childs(@hier, @compar_set, @cost_centre, @kid) as c
        on a.cost_centre = c.cod
        ...
        where a.data_set = @compar_set and a.hier = @hier and
            1 = (case when @cdriver_central is null then 1 else iif(e.central = @cdriver_central, 1, 0) end)
        group by a.coarea, a.hier, a.data_set, d.cost_driver, e.nume, e.central, a.opex_categ, d.nume, g.blocat, a.ic_part, f.nume,
                cast((case when a.coarea = f.coarea then 1 else 0 end) as bit), b.an
