...
@Named(value = "icpart")
@ViewScoped
public class IcPartenerItem implements Serializable, SelectItemView<IcPartener>{
    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(IcPartenerItem.class.getName());
    
    private @Inject @CurrentUser User cuser;
    private @Inject @CurrentLocale Locale clocale;
    
    private @Inject SelectTableView<IcPartener> owner;
    ...
    public void save(){
        try {
            if (this.selected == null || !Utils.stringNotEmpty(this.selected.getCod()))
                throw new Exception(App.getBeanMess("err.icpart.nok", clocale));
            
            if (this.selected.getMod_timp() == null){
                IcPartener rezultat = IcPartenerServ.insert(this.selected, cuser.getUname())
                ...
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.icpart.ins", clocale), App.getBeanMess("info.success",  clocale)));
                
            } else {
                IcPartener rezultat = IcPartenerServ.update(this.selected, cuser.getUname())
                        .orElseThrow(() -> new Exception(App.getBeanMess("err.nosuccess", clocale)));                
                ...
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.icpart.upd", clocale), App.getBeanMess("info.success",  clocale)));
            }
            ...
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, App.getBeanMess("title.icpart.save", clocale), ex.getMessage()));
        }
    }
    ...
    public void delete(){
        try {
            ...
            if (!IcPartenerServ.delete(this.selected.getCod(), cuser.getUname()))
            ...
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.icpart.del", clocale), App.getBeanMess("info.success",  clocale)));
            if (Utils.stringNotEmpty(this.finishScript)) PrimeFaces.current().executeScript(this.finishScript);
            
        } catch (Exception ex) {
            App.log(LOG, Level.SEVERE, cuser.getUname(), ex);
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, App.getBeanMess("title.icpart.del", clocale), ex.getMessage()));
        }
    }
