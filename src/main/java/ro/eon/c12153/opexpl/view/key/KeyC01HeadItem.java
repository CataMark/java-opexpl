...
@Named(value = "kc01h")
@ViewScoped
public class KeyC01HeadItem implements Serializable{
    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(KeyC01HeadItem.class.getName());
    
    private @Inject @CurrentUser User cuser;
    private @Inject @CurrentLocale Locale clocale;
    
    private @Inject KeyC01HeadList owner;
        ...
        CompletableFuture<List<KeyVal>> fValori = CompletableFuture.supplyAsync(() -> {
            ...
            try {
                if (this.selected != null && this.selected.getMod_timp() != null)
                    rezultat = KeyC01Serv.getById(this.selected.getId(), dataset, cuser.getUname())
    ...
    public void save(){
        try {
            ...
            if (this.selected.getMod_timp() == null){
                KeyHead rezultat = KeyC01Serv.insertHead(this.selected, cuser.getUname())
                ...
                KeyHead rezultat = KeyC01Serv.updateHead(this.owner.getDataset().getId(), this.selected, cuser.getUname())
                        .orElseThrow(() -> new Exception(App.getBeanMess("err.nosuccess", clocale)));                
    ...
    public void deleteHead(){
        try {
            ...
            KeyC01Serv.deleteHead(this.selected.getId(), cuser.getUname());            
    ...
    public void checkValsByKey(){
        try{
            if (this.selected == null || this.selected.getMod_timp() == null)
                throw new Exception(App.getBeanMess("err.keys.nok", clocale));
            
            List<KeyHead> rezultat = KeyC01Serv.calcCheckValsByKey(this.selected.getId(), this.owner.getDataset().getId(), this.owner.getCoarea().getCod(), cuser.getUname());
            if (rezultat == null || rezultat.isEmpty()){
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.keys.val.chk", clocale), App.getBeanMess("info.keys.val.ok2",  clocale)));
            } else {
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.keys.val.chk", clocale), App.getBeanMess("info.keys.val.nok2",  clocale)));
            }            
        } catch (Exception ex){
            App.log(LOG, Level.SEVERE, cuser.getUname(), ex);
            FacesContext.getCurrentInstance().addMessage(null,
    ...
    public void calcValsByKey(){
        try {
            if (this.selected == null || this.selected.getMod_timp() == null)
                throw new Exception(App.getBeanMess("err.keys.nok", clocale));
            
            KeyHead rezultat = KeyC01Serv.calcByKey(this.selected.getId(), this.owner.getDataset().getId(), this.owner.getCoarea().getCod(), cuser.getUname())
                    .orElseThrow(() -> new Exception(App.getBeanMess("err.nosuccess", clocale)));                
            for (int i = 0; i < this.owner.getList().size(); i++){
                if (this.owner.getList().get(i).getId().equals(rezultat.getId())){
                    this.owner.getList().set(i, rezultat);
    ...
    public void deleteValsByKey(){
        try {
            if (this.selected == null || this.selected.getValori() == null || this.selected.getValori().isEmpty())
                throw new Exception(App.getBeanMess("err.keys.nok", clocale));
            
            KeyHead rezultat = KeyC01Serv.deleteValsByKey(this.selected.getId(), this.owner.getDataset().getId(), cuser.getUname())
                    .orElseThrow(() -> new Exception(App.getBeanMess("err.nosuccess", clocale)));                
            for (int i = 0; i < this.owner.getList().size(); i++){
                if (this.owner.getList().get(i).getId().equals(rezultat.getId())){
                    this.owner.getList().set(i, rezultat);
