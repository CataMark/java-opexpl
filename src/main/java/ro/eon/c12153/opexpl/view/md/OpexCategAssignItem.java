 ...
 * @author C12153
 */
@Named(value = "ocategas")
@ViewScoped
public class OpexCategAssignItem implements Serializable{
    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(OpexCategAssignItem.class.getName());
    
    private @Inject @CurrentUser User cuser;
    private @Inject @CurrentLocale Locale clocale;
    
    private @Inject OpexCategAssignList owner;
    private String initError;
    private List<OpexCateg> notAssigned;    
    ...
    public void initLists(){
        try {
            if (this.selected.getMod_timp() == null)
                this.notAssigned = OpexCategServ.getNotAssign(this.owner.getCoarea().getCod(), this.owner.getCdriver().getCod(), cuser.getUname());
        } catch (Exception ex) {
    ...
    public void save(){
        try {
            if (this.selected == null || this.selected.getCod() == null)
                throw new Exception(App.getBeanMess("err.opexcat.nok", clocale));
            
            if (this.selected.getMod_timp() == null){
                OpexCateg rezultat = OpexCategServ.insertAssign(this.selected, this.owner.getCoarea().getCod(), cuser.getUname())
                ...
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.opexcat.asg.ins", clocale), App.getBeanMess("info.success",  clocale)));            
            } else {
                OpexCateg rezultat = OpexCategServ.updateAssign(this.selected, cuser.getUname())
                        .orElseThrow(() -> new Exception(App.getBeanMess("err.nosuccess", clocale)));                
                ...
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.opexcat.asg.upd", clocale), App.getBeanMess("info.success",  clocale)));                
            }
            ...
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, App.getBeanMess("title.opexcat.asg.save", clocale), ex.getMessage()));
        }
    }
    ...
    public void delete(){
        try {
            ...
            if (!OpexCategServ.deleteAssign(this.selected.getId(), cuser.getUname()))
            ...
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
                    App.getBeanMess("title.opexcat.asg.del", clocale), App.getBeanMess("info.success",  clocale))); 
            if (Utils.stringNotEmpty(this.finishScript)) PrimeFaces.current().executeScript(this.finishScript);
            
        } catch (Exception ex) {
            App.log(LOG, Level.SEVERE, cuser.getUname(), ex);
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, App.getBeanMess("title.opexcat.asg.del", clocale), ex.getMessage()));
        }
    }
    ...
    public List<OpexCateg> getNotAssigned() {
