...
public class KeyType implements Serializable{
    ...
    public KeyType(Map<String, Object> inreg){
        this.cod = (String) inreg.get(COD);
        this.nume = (String) inreg.get(NUME);
        this.general = (Boolean) inreg.get(GENERAL);
        this.calculat = (Boolean) inreg.get(CALCULAT);
        this.mod_de = (String) inreg.get(MOD_DE);
        this.mod_timp = (Date) inreg.get(MOD_TIMP);
    }
    
    public KeyType(String json, boolean encoded) throws Exception{
    ...
    public KeyType(JsonObject json) throws Exception{
        this.parseJson(json);
    }
    ...
    private void parseJson(JsonObject jsonO) throws Exception{
        ...
        if (jsonO.containsKey(GENERAL) && !jsonO.isNull(GENERAL)) this.general = jsonO.getBoolean(GENERAL);
        if (jsonO.containsKey(CALCULAT) && !jsonO.isNull(CALCULAT)) this.calculat = jsonO.getBoolean(CALCULAT);
    ...
    public JsonObject getJson(){
        ...
        if (this.general != null) jsonb.add(GENERAL, this.general);        
        if (this.calculat != null) jsonb.add(CALCULAT, this.calculat);        
    ...
    public Boolean getCalculat() {
        return calculat;
    }

    public void setCalculat(Boolean calculat) {
